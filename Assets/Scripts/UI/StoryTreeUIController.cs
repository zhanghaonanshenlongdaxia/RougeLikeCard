...

{
...

    {
...

        {
            ...
            _contentRoot = panel.Find("StoryScroll/Viewport/Content");


            var detailGO = panel.Find("DetailPanel")?.gameObject;

            _detailPanel = detailGO;
...

            {
                ...
                var texts = _detailPanel.GetComponentsInChildren<TMPro.TextMeshProUGUI>(true);

                if (texts.Length >= 3) { _detailTitle = texts[0]; _detailDesc = texts[1]; _detailReward = texts[2]; }

                var btnRow = _detailPanel.transform.Find("BtnRow");

                if (btnRow != null)

                {

                    _unlockBtn = btnRow.Find("Btn_解锁")?.GetComponent<UnityEngine.UI.Button>();

                    _closeDetailBtn = btnRow.Find("Btn_关闭")?.GetComponent<UnityEngine.UI.Button>();
