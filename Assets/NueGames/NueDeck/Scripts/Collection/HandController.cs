...
namespace NueGames.NueDeck.Scripts.Collection
{
    public class HandController : MonoBehaviour, IController
    {
        ...
        protected NueUIManager UIManager => NueUIManager.Instance;
        ...
        private void HandleCardsInHand(int count, bool mouseButton, float sqrDistance)
        {
            for (var i = 0; i < count; i++)
            {
                ...
                card.SetInactiveMaterialState(this.GetModel<IBattleModel>().CurrentMana.Value < card.CardData.ManaCost);
                ...
                // Get Selected Card
                if (this.GetModel<IBattleModel>().CanSelectCards.Value)
                {
        ...
        private void HandleDraggedCardOutsideHand(bool mouseButton, Vector2 mousePos)
        {
            if (_heldCard != null)
            {
                ...
                if (!this.GetModel<IBattleModel>().CanSelectCards.Value || _mouseInsideHand)
                {
                    //  || sqrDistance <= 2
        ...
        private void PlayCard(Vector2 mousePos)
        {
            ...
            if (this.GetModel<IBattleModel>().CanUseCards.Value && this.GetModel<IBattleModel>().CurrentMana.Value >= _heldCard.CardData.ManaCost)
            {
                RaycastHit hit;
